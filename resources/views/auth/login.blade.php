        ...
        <div class="bg-white/90 dark:bg-gray-800/80 rounded-3xl shadow-2xl p-10 backdrop-blur-lg border border-gray-200 dark:border-gray-700">
            <div class="flex flex-col items-center mb-8">
                <a href="{{ url('/') }}" class="mb-3">
                    <img src="{{ asset('storage/images/ravinallogo.jpg') }}" alt="PROFCHECK" class="w-24 h-24 rounded-full object-cover shadow-lg border-4 border-indigo-400 dark:border-indigo-700" />
                </a>
            ...
            @if (session('status'))
                <div class="mb-4 text-sm text-green-600 bg-green-50 border border-green-200 rounded px-4 py-2">{{ session('status') }}</div>
            @endif
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-sm text-indigo-600 hover:underline font-medium">Forgot password?</a>
                        @endif
                    </div>
                </div>
