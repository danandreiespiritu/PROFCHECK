                ...
                <table class="min-w-full bg-white text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-t hover:bg-blue-50 transition">
                            <td class="px-4 py-3">{{ \Carbon\Carbon::parse($log->date)->format('M d, Y') }}</td>
                            <td class="px-4 py-3">{{ $log->time_in ? \Carbon\Carbon::parse($log->time_in)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-3">{{ $log->time_out ? \Carbon\Carbon::parse($log->time_out)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <span class="inline-block px-2 py-1 text-xs font-bold bg-gray-100 text-gray-700 rounded">{{ $log->status }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">{{ $log->classSchedule?->subject ?? '-' }}</td>
                        </tr>
                        @empty
                        <tr>
