                ...
                <table class="min-w-full divide-y divide-blue-100 text-sm">
                    ...
                    <tbody class="bg-white divide-y divide-blue-50">
                        ...
                        <tr class="hover:bg-blue-50 transition group">
                            <td class="px-5 py-4 whitespace-nowrap text-blue-900 font-semibold">{{ $faculties->Faculty_ID }}</td>
                            <td class="px-5 py-4 whitespace-nowrap">
                                <span class="text-blue-900">{{ $faculties->FirstName }} {{ $faculties->LastName }}</span>
                            </td>
                            <td class="px-5 py-4 whitespace-nowrap text-blue-700">{{ $faculties->Email }}</td>
                            <td class="px-5 py-4 whitespace-nowrap text-blue-700">{{ $faculties->Position }}</td>
                            <td class="px-5 py-4 whitespace-nowrap text-blue-700">{{ $faculties->rfid_tag }}</td>
                            <td class="px-5 py-4 whitespace-nowrap text-blue-700">{{ $faculties->Gender }}</td>
                            <td class="px-5 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('admin.manageFaculty.editFaculty', $faculties->Faculty_ID) }}"
                                    ...
                                    <button type="button"
                                            ...
                                            onclick="openDeleteModal('{{ $faculties->Faculty_ID }}')"
                                ...
                                <!-- Delete Modal -->
                                <div id="deleteModal-{{ $faculties->Faculty_ID }}" class="fixed inset-0 flex items-center justify-center z-50 hidden">
                                    ...
                                    <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full space-y-6 border border-blue-100 animate-fade-in">
                                        ...
                                        <p class="text-gray-600 text-base">
                                            ...
                                            <span class="font-semibold text-blue-700">{{ $faculties->FirstName }} {{ $faculties->LastName }}</span>?
                                            <br>
                                            <span class="text-sm text-red-600 font-semibold">This action cannot be undone.</span>
                                        </p>
                                        <div class="flex justify-end gap-3 pt-2">
                                            <button type="button"
                                                    onclick="closeDeleteModal('{{ $faculties->Faculty_ID }}')"
                                            ...
                                            <form action="{{ route('admin.manageFaculty.deleteFaculty', $faculties->Faculty_ID) }}"
                                                  method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
