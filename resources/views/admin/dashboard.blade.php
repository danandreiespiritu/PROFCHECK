 ...
 <body x-data="{ sidebarOpen: true }" class="bg-[#f0f5f8] min-h-screen flex">
   ...
   <!-- Dashboard content -->
        ...
        <div class="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg border border-blue-100 p-6 md:p-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                ...
                <div class="flex items-center bg-gradient-to-r from-blue-100 to-blue-50 rounded-xl p-5 shadow hover:shadow-md transition">
                    ...
                    <div>
                        <div class="text-xl font-bold text-blue-800">{{ $facultyCount ?? 0 }}</div>
                ...
                <div class="flex items-center bg-gradient-to-r from-yellow-100 to-yellow-50 rounded-xl p-5 shadow hover:shadow-md transition">
                    ...
                    <div>
                        <div class="text-xl font-bold text-yellow-800">{{ $todayAttendance ?? 0 }}</div>
                ...
                <div class="flex items-center bg-gradient-to-r from-green-100 to-green-50 rounded-xl p-5 shadow hover:shadow-md transition">
                    ...
                    <div>
                        <div class="text-xl font-bold text-green-800">{{ $classesScheduledToday ?? 0 }}</div>
                ...
                <div class="flex items-center bg-gradient-to-r from-red-100 to-red-50 rounded-xl p-5 shadow hover:shadow-md transition">
                    ...
                    <div>
                        <div class="text-xl font-bold text-red-800">{{ $lateArrivalsToday ?? 0 }}</div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm text-blue-900">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @foreach($trendLabels as $i => $day)
                                    <tr class="border-b last:border-0">
                                        <td class="px-3 py-2">{{ $day }}</td>
                                        <td class="px-3 py-2">{{ $trendData[$i] ?? 0 }}</td>
                                        <td class="px-3 py-2">{{ max(0, $facultyTotal - ($trendData[$i] ?? 0)) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="bg-white rounded-xl p-6 shadow border flex flex-col justify-center items-center">
                    ...
                    <div class="mt-4 flex flex-col items-center">
                        <span class="text-2xl font-bold text-blue-700">{{ $percent }}%</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl p-6 shadow border">
                    ...
                    <ul class="divide-y divide-blue-100">
                        @forelse($recentFacultyActivity ?? [] as $activity)
                            <li class="py-2 flex items-center gap-2">
                                <i class="fas fa-user-circle text-blue-400"></i>
                                <span class="font-semibold text-blue-800">{{ $activity['name'] ?? 'Unknown' }}</span>
                                <span class="text-xs text-blue-400 ml-2">{{ $activity['action'] ?? '' }}</span>
                                <span class="ml-auto text-xs text-blue-300">{{ $activity['time'] ?? '' }}</span>
                            </li>
                        @empty
                    ...
                    <ul class="divide-y divide-blue-100">
                        @forelse($upcomingSchedules ?? [] as $sched)
                            <li class="py-2 flex flex-col">
                                <span class="font-semibold text-blue-800">{{ $sched['subject'] ?? 'Class' }}</span>
                                <span class="text-xs text-blue-400">{{ $sched['faculty'] ?? '' }}</span>
                                <span class="text-xs text-blue-300">{{ $sched['time'] ?? '' }}</span>
                            </li>
                        @empty
    ...
    <script>
        ...
        const attendanceTrendLabels = {!! json_encode($attendanceTrendLabels ?? ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']) !!};
        const attendanceTrendData = {!! json_encode($attendanceTrendData ?? [30, 45, 50, 40, 60, 55, 70]) !!};
        const facultyAttendanceRate = {!! json_encode($facultyAttendanceRate ?? ['Present' => 80, 'Absent' => 20]) !!};
